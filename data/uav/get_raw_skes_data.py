...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    frame_length.append(num_frames)
    frames_drop = []
    bodies_data = dict()
...
if __name__ == '__main__':
    ...
    skes_path = '/data2/jiangzhen/sjl/GCN/data/uav/Skeleton/'
    stat_path = osp.join(save_path, 'statistics')
    ...
    file = open('/data2/jiangzhen/sjl/GCN/data/uav/Skeleton/ntu_frame.txt', 'w')

    file.truncate(0)

    for key in frame_length:
        file.write(str(key))
        file.write(' ')
