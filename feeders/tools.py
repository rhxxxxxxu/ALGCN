...
def valid_crop_resize(data_numpy, valid_frame_num, p_interval, window):
    # input: C,T,V,M (C: channels, T: frames, V: vertices/joints, M: persons/subjects)
    C, T, V, M = data_numpy.shape
    begin = 0
    end = valid_frame_num
    valid_size = end - begin
    ...
    if len(p_interval) == 1:
        p = p_interval[0]
        bias = int((1 - p) * valid_size / 2)
        cropped_length = valid_size - 2 * bias  # Ensure length after crop is valid
        if cropped_length <= 0:
            print(f"Skipping data: invalid cropped_length={cropped_length}.")
            return None  # 跳过无效数据
        data = data_numpy[:, begin + bias:end - bias, :, :]  # Center crop
    else:
        p = np.random.rand(1) * (p_interval[1] - p_interval[0]) + p_interval[0]
        cropped_length = np.minimum(np.maximum(int(np.floor(valid_size * p)), 64), valid_size)  # lower bound: 64
        if cropped_length <= 0:
            print(f"Skipping data: invalid cropped_length={cropped_length}.")
            return None  # 跳过无效数据
        bias = np.random.randint(0, valid_size - cropped_length + 1)
    ...
    if data.shape[1] == 0:
        print(
            f"Skipping data: zero-length after cropping, cropped_length={cropped_length}, bias={bias}, valid_size={valid_size}")
        return None  # 跳过无效数据

    # Resize
    data = torch.tensor(data, dtype=torch.float32)
    data = data.permute(0, 2, 3, 1).contiguous().view(C * V * M, cropped_length)  # Reshape for interpolation
    data = data[None, None, :, :]  # Add batch and channel dimensions

    # 检查插值前的尺寸是否合法
    if cropped_length == 0 or C * V * M == 0:
        print(
            f"Skipping data: invalid dimensions before interpolation: cropped_length={cropped_length}, C*V*M={C * V * M}")
        return None  # 跳过无效数据

    # Interpolate
    data = F.interpolate(data, size=(C * V * M, window), mode='bilinear',
                         align_corners=False).squeeze()  # Interpolation
    data = data.contiguous().view(C, V, M, window).permute(0, 3, 1, 2).contiguous().numpy()
